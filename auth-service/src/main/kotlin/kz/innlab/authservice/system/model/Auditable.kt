package kz.innlab.authservice.system.model
...
abstract class Auditable<U> : Serializable {
    ...
    var eventLog: ArrayList<kz.innlab.authservice.auth.dto.EventLogs>? = arrayListOf()
        get() {
            return field?: arrayListOf()
        }
        private set

    fun addLog(value: kz.innlab.authservice.auth.dto.EventLogs) {
        if (eventLog != null) {
            eventLog!!.add(value)
        } else {
